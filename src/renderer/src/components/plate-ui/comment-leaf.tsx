...
export function CommentLeaf({ className, ...props }: PlateLeafProps<Value, TCommentText>) {
  ...
  if (!state.isActive) {
    for (let i = 1; i < state.commentCount; i++) {
      aboveChildren = (
        <span className="bg-neutral-900/20 dark:bg-neutral-50/20">{aboveChildren}</span>
      )
    }
  }
  ...
  return (
    <PlateLeaf
      ...
      className={cn(
        ...
        state.isActive
          ? 'bg-neutral-900/40 dark:bg-neutral-50/40'
          : 'bg-neutral-900/20 dark:bg-neutral-50/20',
        className
      )}
      nodeProps={{
        ...rootProps,
